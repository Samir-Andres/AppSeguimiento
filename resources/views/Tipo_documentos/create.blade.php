...
@section('title', 'Crear tipo de documentos')
...
@section('content_header')
    ...
    <div class="flex flex-wrap items-center space-x-2 text-sm text-gray-500 font-medium">
        ...
        <a class="text-indigo-500">Crear tipo de documento</a>
...
@section('content')
    ...
    <form action="{{route('TipoDocumentos.store')}}" method="POST" id="main-form" style="display: none;" class="flex flex-col items-center text-sm text-slate-800 p-6 bg-white shadow-xl rounded-2xl border border-slate-100">
        @csrf


        <h1 class="text-4xl font-bold py-4 text-center">Crear tipo de documentos</h1>
        ...
        <div class="max-w-96 w-full px-4">
            ...
            <div
                ...
                <input type="text" name="Denominacion" class="h-full px-2 w-full outline-none bg-transparent"
                       placeholder="Ejem: CC" value="{{ old('Denominacion') }}">
            </div>
            @error('Denominacion')
            <p class="text-red-500 text-xs mb-0 pl-2 text-center">{{ $message }}</p>
            @enderror

            <label for="Observaciones" class="font-medium mt-4 block">Descripción del tipo</label>
            <textarea name="Observaciones" rows="4"
                      ...
                      placeholder="Ejem: Cedula de ciudadanía">{{ old('Observaciones') }}</textarea>
