...
@section('content_header')
    ...
    <div class="flex flex-wrap items-center space-x-2 text-sm text-gray-500 font-medium">
        ...
        <a href="{{ route('TipoDocumentos.index') }}">Tipo de documentos</a>
        <svg width="20" height="20" viewBox="0 0 20 20" fill="none" xmlns="http://www.w3.org/2000/svg">
            <path
                d="m14.413 10.663-6.25 6.25a.939.939 0 1 1-1.328-1.328L12.42 10 6.836 4.413a.939.939 0 1 1 1.328-1.328l6.25 6.25a.94.94 0 0 1-.001 1.328"
                fill="#CBD5E1" />
        </svg>
        <a class="text-indigo-500">Editar tipo de documentos</a>
...
@section('content')
    ...
    <form action="{{route('TipoDocumentos.update', $tipo->NIS)}}" method="POST" id="main-form" style="display: none;" class="flex flex-col items-center text-sm text-slate-800 p-6 bg-white shadow-xl rounded-2xl border border-slate-100">
        @csrf
        @method('PUT')

        <h1 class="text-4xl font-bold py-4 text-center">Editar Tipo de documento</h1>

        <div class="max-w-96 w-full px-4">

            <label for="Denominacion" class="font-medium">Nombre del tipo</label>
            <div
                class="flex items-center mt-2 mb-1 h-10 pl-3 border {{ $errors->has('Denominacion') ? 'border-red-500' : 'border-slate-300' }} rounded-full focus-within:ring-2 focus-within:ring-indigo-400 transition-all overflow-hidden">
                <input type="text" name="Denominacion" class="h-full px-2 w-full outline-none bg-transparent"
                       placeholder="Ejem: CC" value="{{ old('Denominacion', $tipo->Denominacion) }}">
            </div>
            @error('Denominacion')
            <p class="text-red-500 text-xs mb-0 pl-2 text-center">{{ $message }}</p>
            @enderror

            <label for="Observaciones" class="font-medium mt-4 block">Descripción del rol</label>
            <textarea name="Observaciones" rows="4"
                      ...
                      placeholder="Ejem: Cedula de ciudadanía">{{ old('Observaciones', $tipo->Observaciones) }}</textarea>
