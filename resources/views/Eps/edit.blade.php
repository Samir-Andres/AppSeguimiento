...
@section('content_header')
    ...
    <div class="flex flex-wrap items-center space-x-2 text-sm text-gray-500 font-medium">
        ...
        <a href="{{route('Alternativa.Index')}}" >Eps</a>
        <svg width="20" height="20" viewBox="0 0 20 20" fill="none" xmlns="http://www.w3.org/2000/svg">
            <path d="m14.413 10.663-6.25 6.25a.939.939 0 1 1-1.328-1.328L12.42 10 6.836 4.413a.939.939 0 1 1 1.328-1.328l6.25 6.25a.94.94 0 0 1-.001 1.328" fill="#CBD5E1"/>
        </svg>
        <a  class="text-indigo-500">Editar eps</a>
...
@section('content')
    ...
    <form action="{{ route('eps.update', $eps->NIS) }}" method="POST" id="main-form"
          style="display: none;"
          class="flex flex-col items-center text-sm text-slate-800 p-6 bg-white shadow-xl rounded-2xl border border-slate-100">
        @csrf
        @method('PUT')

        <h1 class="text-4xl font-bold py-4 text-center">Editar eps</h1>

        <div class="max-w-96 w-full px-4">
            <!-- Campo Nombre -->
            <label for="Numdoc" class="font-medium">Numero de la documento</label>
            <div
                class="flex items-center mt-2 mb-1 h-10 pl-3 border {{ $errors->has('Numdoc') ? 'border-red-500' : 'border-slate-300' }} rounded-full focus-within:ring-2 focus-within:ring-indigo-400 transition-all overflow-hidden">
                {{-- <i class="fa-solid fa-tag text-slate-500"></i> --}}
                <input type="number" name="Numdoc" value="{{ old('Numdoc', $eps->Numdoc) }}"
            ...
            <!-- Campo Estado -->
            <label for="Denominacion" class="font-medium mt-4 block">Estado del Registro</label>
            <div
                class="flex items-center mt-2 mb-1 h-10 pl-3 border border-slate-300 {{ $errors->has('Denominacion') ? 'border-red-500' : 'border-slate-300' }} rounded-full focus-within:ring-2 focus-within:ring-indigo-400 transition-all overflow-hidden">
                <input type="text" name="Denominacion" value="{{ old('Denominacion', $eps->Denominacion) }}"
            ...
            <!-- Campo Descripción -->
            <label for="Observaciones" class="font-medium mt-4 block">Observaciones</label>
            <textarea name="Observaciones" rows="4"
                      ...
                      placeholder="Detalles de la eps">{{ old('Observaciones', $eps->Observaciones) }}</textarea>
