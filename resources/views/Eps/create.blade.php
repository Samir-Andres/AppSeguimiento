...
@section('content_header')
    <div class="flex flex-wrap  items-center space-x-2 text-sm text-gray-500 font-medium">
        ...
        <a class="text-indigo-500">Crear Eps</a>
...
@section('content')
    ...
    <form action="{{route('eps.store')}}" class="flex flex-col items-center text-sm text-slate-800" method="POST">
        @csrf
        <h1 class="text-4xl font-bold py-4 text-center">Crear eps</h1>

        <div class="max-w-96 w-full px-4">
            <label for="name" class="font-medium">N. Documentos</label>
            <div class="flex items-center mt-2 mb-2 h-10 pl-3 border border-slate-300 rounded-full focus-within:ring-2 focus-within:ring-indigo-400 transition-all overflow-hidden">
                <input type="text" name="Numdoc" class="h-full px-2 w-full outline-none bg-transparent" placeholder="Documentos" value="{{old('Codigo')}}" >
            ...
            <div class="flex items-center mt-2 mb-2 h-10 pl-3 border border-slate-300 rounded-full focus-within:ring-2 focus-within:ring-indigo-400 transition-all overflow-hidden">
                <input type="text" name="Denominacion" class="h-full px-2 w-full outline-none bg-transparent" placeholder="Denominación" value="{{old('Denominacion')}}">
            ...
            <button type="submit" class="flex items-center justify-center gap-1 mt-3 bg-indigo-500 hover:bg-indigo-600 text-white py-2.5 w-full rounded-full transition">
                Crear
            </button>
        </div>
    </form>
