...
@section('content_header')
    ...
    <div class="flex flex-wrap items-center space-x-2 text-sm text-gray-500 font-medium">
        ...
        <a class="text-indigo-500">Editar bitácora</a>
...
@section('content')
    ...
    <form action="{{route('Bitacoras.update', $bitacora->id)}}" method="POST" id="main-form" style="display: none;" class="flex flex-col items-center text-sm text-slate-800 p-6 bg-white shadow-xl rounded-2xl border border-slate-100" enctype="multipart/form-data">
        @csrf

        @method('PUT')


        <h1 class="text-4xl font-bold py-4 text-center">Reemplazar bitácora</h1>
        ...
        <div class="max-w-md w-full p-6 bg-white rounded-lg border border-gray-500/30 shadow-[0px_1px_15px_0px] shadow-black/10 text-sm">
            <label for="fileInput" class="border-2 border-dotted border-gray-400 p-8 mt-2 flex flex-col items-center gap-4 cursor-pointer hover:border-blue-500 transition">
                ...
                <div id="nombreArchivo" class="archivo-nombre">
                    ...
                    @if($bitacora->file)
                        <a href="{{asset($bitacora->file)}}" target="_blank" class="flex flex-col items-center p-0 text-center">
                        {{ basename($bitacora->file) }}
                            <span class="">Ver</span>
                        </a>
                    @else
            ...
            <div class="mt-6 flex justify-end gap-4">
                <a href="{{route('Bitacoras.index')}}" type="reset" class="px-9 py-2 border border-gray-500/50 bg-white hover:bg-blue-100/30 active:scale-95 transition-all text-gray-500 rounded">
                    Cancel
                </a>
