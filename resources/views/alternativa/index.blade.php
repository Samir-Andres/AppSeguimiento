...
@section('content_header')
    ...
    <div class="flex flex-wrap  items-center space-x-2 text-sm text-gray-500 font-medium">
        ...
        <a class="text-indigo-500">Alternativa</a>
    ...
    <div class="relative isolate flex items-center gap-x-6 overflow-hidden  px-6 py-2.5 sm:px-3.5 sm:before:flex-1">
        ...
        <div class="flex flex-wrap items-center gap-x-4 gap-y-2">
            <p class="text-sm/6 text-gray-900">
                <strong class="font-semibold uppercase">Centro de registro </strong>Puedes registrar alternativas
            </p>
            <a href="{{ route('create.alternativa') }}"
                class="flex-none rounded-full bg-gray-900 px-3.5 py-1 text-sm font-semibold text-white shadow-xs hover:bg-gray-700 focus-visible:outline-2 focus-visible:outline-offset-2 focus-visible:outline-gray-900">Registrar
                alternativa</a>
        </div>
            ...
            <table class="w-full text-sm text-left text-gray-500">
                <thead class="text-xs text-gray-700 uppercase bg-gray-50">
                    <tr>
                        ...
                        <th scope="col" class="px-6 py-3">Actualizacion</th>
                        <th scope="col" class="px-6 py-3">Acción</th>
                    </tr>
                </thead>
                <tbody>

                    @forelse ($alternativa as $alternativas)
                        <tr class="bg-white border-b hover:bg-gray-50">
                            ...
                            <td class="px-6 py-3 whitespace-nowrap">
                                ...
                                @if ($alternativas->descripcion)
                                    <span class="text-gray-900 font-medium">{{ $alternativas->descripcion }}</span>
                                @else
                            ...
                            <td class="px-6 py-3 whitespace-nowrap">
                                {{ $alternativas->password }}
                            </td>

                            <td class="px-6 py-3 whitespace-nowrap">
                                <a href="" title="Fecha y horas exactas">
                                    <span>{{ $alternativas->created_at->diffForHumans() }}</span>
                                </a>
                            ...
                            <td class="px-6 py-3">
                                <a href="" title="Fecha y horas exactas">
                                    <span>{{ $alternativas->updated_at->diffForHumans() }}</span>
                                </a>
                            </td>
                            ...
                            <td class="px-10 py-3">
                                <div class="dropdown">
                                    ...
                                    <div class="dropdown-menu dropdown-menu-right shadow-lg border-0 p-2">
                                        ...
                                        <a href="{{route('alternativa.show', $alternativas->id)}}"
                                        ...
                                        <a href="{{ route('alternativas.edit', $alternativas->id) }}"
                                        ...
                                        <form action="{{ route('alternativas.delete', $alternativas->id) }}" method="POST"
                                            class="form-eliminar">
                                            @csrf
                                            @method('DELETE')
                                            <button type="submit"
