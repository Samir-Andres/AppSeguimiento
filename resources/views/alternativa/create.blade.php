...
@section('content_header')
    ...
    <div class="flex flex-wrap items-center space-x-2 text-sm text-gray-500 font-medium">
        ...
        <a class="text-indigo-500">Crear alternativa</a>
...
@section('content')
    ...
    <form action="{{route('alternativa.store')}}" method="POST" id="main-form" style="display: none;" class="flex flex-col items-center text-sm text-slate-800 p-6 bg-white shadow-xl rounded-2xl border border-slate-100">
        @csrf


        <h1 class="text-4xl font-bold py-4 text-center">Crear Alternativa</h1>
        ...
        <div class="max-w-96 w-full px-4">
            ...
            <div
                ...
                <input type="text" name="nombre" class="h-full px-2 w-full outline-none bg-transparent"
                    placeholder="Eje: Proyecto productivo" value="{{ old('nombre') }}">
            </div>
            @error('nombre')
            ...
            <div
                class="flex items-center mt-2 mb-1 h-10 pl-3 border {{ $errors->has('password') ? 'border-red-500' : 'border-slate-300' }} rounded-full focus-within:ring-2 focus-within:ring-indigo-400 transition-all overflow-hidden">
                <input type="text" name="password" class="h-full px-2 w-full outline-none bg-transparent" placeholder="**********">
            </div>
            @error('password')
            <p class="text-red-500 text-xs mb-0 pl-2 text-center">{{ $message }}</p>
            @enderror
            ...
            <textarea name="descripcion" rows="4"
                ...
                placeholder="Detalles de la alternativa">{{ old('descripcion') }}</textarea>
