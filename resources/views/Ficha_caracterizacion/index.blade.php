...
@section('title', 'Ficha de caracterización')
...
@section('content_header')
    ...
    <div class="flex flex-wrap  items-center space-x-2 text-sm text-gray-500 font-medium">
        ...
        <a class="text-indigo-500">Fichas</a>
        <svg width="20" height="20" viewBox="0 0 20 20" fill="none" xmlns="http://www.w3.org/2000/svg">
            <path
                d="m14.413 10.663-6.25 6.25a.939.939 0 1 1-1.328-1.328L12.42 10 6.836 4.413a.939.939 0 1 1 1.328-1.328l6.25 6.25a.94.94 0 0 1-.001 1.328"
                fill="#CBD5E1" />
        ...
        <a href="" class="text-green-500 hover:no-underline" title="Cargar excel">
            <i class="fas fa-file-csv text-[20px] "></i>
        </a>
    ...
    <div class="relative isolate flex items-center gap-x-6 overflow-hidden px-6 py-2.5 sm:px-3.5 sm:before:flex-1">
        ...
        <div class="flex items-center gap-x-4">
            <div class="text-sm text-gray-900 text-justify md:text-left leading-relaxed">
                <strong class="font-semibold uppercase">Bienvenidos al centro de fichas de caracterización,</strong>
                <span>puedes crear eliminar y actualizar</span>

                <a href="{{route('Ficha_caracterizacion.create')}}"
                ...
                @forelse ($ficha as $item)
                    <tr class="bg-white border-b hover:bg-gray-50">
                        ...
                        <td class="px-6 py-3 whitespace-nowrap">
                            <span class="text-gray-900 font-medium">{{$item->Codigo}}</span>
                        </td>


                        <td class="px-6 py-3 whitespace-nowrap">
                            <span class="text-gray-900 font-medium">{{$item->Denominacion}}</span>
                        </td>

                        <td class="px-6 py-3 whitespace-nowrap">
                            <span class="text-gray-900 font-medium">{{$item->Cupos}}</span>
                        </td>

                        <td class="px-6 py-3 whitespace-nowrap">
                            <span class="text-gray-900 font-medium">{{$item->Fecha_Inicio}}</span>
                        </td>

                        <td class="px-6 py-3 whitespace-nowrap">
                            <span class="text-gray-900 font-medium">{{$item->Fecha_Fin}}</span>
                        </td>
                        ...
                        <td class="px-6 py-3 whitespace-nowrap">
                            <span class="text-gray-900 font-medium">{{$item->centro_formacion->Denominacion}}</span>
                        </td>

                        <td class="px-6 py-3 whitespace-nowrap">
                            <span class="text-gray-900 font-medium">{{$item->instructores->Nombres}}</span>
                        </td>
                        ...
                        <td class="px-10 py-3">
                            <div class="dropdown">
                                ...
                                <div class="dropdown-menu dropdown-menu-right shadow-lg border-0 p-2">
                                    ...
                                    <a href="{{route('Ficha_caracterizacion.show', $item->NIS)}}"
                                    ...
                                    <a href="{{route('Ficha_caracterizacion.edit', $item->NIS)}}"
                                    ...
                                    <form action="{{route('Ficha_caracterizacion.destroy', $item->NIS)}}" method="POST"
                                          class="form-eliminar">
                                        @csrf
                                        @method('DELETE')
                                        <button type="submit"
