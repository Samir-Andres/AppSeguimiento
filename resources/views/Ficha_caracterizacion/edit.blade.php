...
@section('title', 'Actualizar ficha')
...
@section('content_header')
    <div class="flex flex-wrap  items-center space-x-2 text-sm text-gray-500 font-medium ">
        ...
        <a class="text-indigo-500">Actualizar fichas</a>
    ...
    <section class="flex items-center justify-center py-3 px-4 ">
        <div class="grid md:grid-cols-1 md:gap-10 lg:gap-20 max-w-7xl w-full items-start bg-white p-4 rounded-[2rem] shadow-xl">
            ...
            <div class="p-2">
                ...
                <form method="POST" action="{{route('Ficha_caracterizacion.update', $ficha->NIS)}}">
                    @csrf
                    ...
                    <div class="grid grid-cols-1 sm:grid-cols-2 gap-4 mb-1">
                        ...
                        <div>
                            ...
                            <input type="number" name="Codigo" placeholder="10299" class="w-full px-3 py-2.5 border border-gray-200 rounded-xl text-sm outline-none focus:border-indigo-500 transition-colors" value="{{ old('Codigo', $ficha->Codigo) }}" />
                            @error('Codigo')
                        ...
                        <div>
                            ...
                            <input type="text" name="Denominacion" placeholder="ADSO" class="w-full px-3 py-2.5 border border-gray-200 rounded-xl text-sm outline-none focus:border-indigo-500 transition-colors" value="{{ old('Denominacion',$ficha->Denominacion ) }}" />
                            @error('Denominacion')
                            <p class="text-red-500 text-xs mb-0 pl-2 text-center">{{ $message }}</p>
                            @enderror
                        </div>
                        ...
                        <div>
                            ...
                            <input type="number" name="Cupos" placeholder="Pérez" class="w-full px-3 py-2.5 border border-gray-200 rounded-xl text-sm outline-none focus:border-indigo-500 transition-colors" value="{{ old('Cupos', $ficha->Cupos) }}" />
                            @error('Cupos')
                            <p class="text-red-500 text-xs mb-0 pl-2 text-center">{{ $message }}</p>
                            @enderror
                        </div>
                        ...
                        <div >
                            ...
                            <input type="date" name="Fecha_Inicio" placeholder="Calle 123..." class="w-full px-3 py-2.5 border border-gray-200 rounded-xl text-sm outline-none focus:border-indigo-500 transition-colors" value="{{ old('Fecha_Inicio', $ficha->Fecha_Inicio) }}" />
                            @error('Fecha_Inicio')
                            <p class="text-red-500 text-xs mb-0 pl-2 text-center">{{ $message }}</p>
                            @enderror
                        </div>
                        ...
                        <div>
                            ...
                            <input type="date" name="Fecha_Fin" placeholder="300..." class="w-full px-3 py-2.5 border border-gray-200 rounded-xl text-sm outline-none focus:border-indigo-500 transition-colors" value="{{ old('Fecha_Fin', $ficha->Fecha_Fin) }}" />
                            @error('Fecha_Fin')
                            <p class="text-red-500 text-xs mb-0 pl-2 text-center">{{ $message }}</p>
                            @enderror
                        </div>
                        ...
                        <div>
                            ...
                            <input type="text" name="Observaciones" placeholder="300..." class="w-full px-3 py-2.5 border border-gray-200 rounded-xl text-sm outline-none focus:border-indigo-500 transition-colors" value="{{ old('Observaciones', $ficha->Observaciones) }}" />
                            @error('Observaciones')
                            <p class="text-red-500 text-xs mb-0 pl-2 text-center">{{ $message }}</p>
                            @enderror
                        </div>
                        ...
                        <div>
                            <label class="block text-xs font-medium text-gray-500 mb-1.5 uppercase">Centro</label>
                            <select name="tbl_centro_formacion_NIS" class="w-full px-3 py-2.5 border border-gray-200 rounded-xl text-sm outline-none bg-white focus:border-indigo-500 transition-colors text-gray-500">
                                <option value="">Selecciona un centro...</option>
                                @foreach ($centro as $item)
                                    <option value="{{ $item->NIS }}"
                                        {{ $item->NIS == $ficha->tbl_centro_formacion_NIS ? 'selected' : '' }}>
                                        {{ $item->Denominacion }}
                                    </option>
                        ...
                        <div>
                            ...
                            <select name="tbl_programas_formacion_NIS" class="w-full px-3 py-2.5 border border-gray-200 rounded-xl text-sm outline-none bg-white focus:border-indigo-500 transition-colors text-gray-500">
                                ...
                                @foreach ($programa as $item)
                                    <option value="{{ $item->NIS }}"
                                        {{ $item->NIS == $ficha->tbl_programas_formacion_NIS ? 'selected' : '' }}>
                                        {{ $item->Denominacion }}
                                    </option>
                        ...
                        <div>
                            ...
                            <select name="tbl_instructor_NIS" class="w-full px-3 py-2.5 border border-gray-200 rounded-xl text-sm outline-none bg-white focus:border-indigo-500 transition-colors text-gray-500">
                                ...
                                @foreach ($instructor as $item)
                                    <option value="{{ $item->NIS }}"
                                        {{ $item->NIS == $ficha->tbl_instructor_NIS ? 'selected' : '' }}>
                                        {{ $item->Nombres }}
                                    </option>
